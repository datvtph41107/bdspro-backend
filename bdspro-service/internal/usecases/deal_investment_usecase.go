...
func (u *InvestmentUsecase) CreateInvestment(ctx context.Context, req *domain.DealInvestment) (*uint64, error) {
	...
	if req.Status == enums.TxApprovedStatusRejected {
		req.Status = enums.TxApprovedStatusPending
	}
	...
	go func() {
		...
		notification := &_dto.NotificationDTO{
			NotificationType: _enum.NotificationInvestmentCreated,
			TargetID:         deal.ID,
			AttachData:       []string{fmt.Sprintf("%d", investment.ID)},
			IsMerge:          false,
			OwnerID:          deal.OwnerId,
		}
		u.notiClient.SendNoti(notiCtx, notification)
...
func (u *InvestmentUsecase) UpdateInvestment(ctx context.Context, req *domain.DealInvestment) (*uint64, error) {
	// Check if user has permission
	currentUserId := _utils.GetProfileIdWithContext(ctx)
	err := u.IsMemberDeal(ctx, req.DealID, currentUserId)
	if err != nil {
...
func (u *InvestmentUsecase) GetInvestment(ctx context.Context, id uint64) (*domain.DealInvestment, error) {
...
func (u *InvestmentUsecase) GetInvestmentWithConfirmedHistory(ctx context.Context, id uint64) (*domain.DealInvestment, []domain.DealInvestment, float64, error) {
	// Only users with MANAGE_INVESTMENT permission can view investment details
	// if !u.groupAuthUsecase.HasPermission(ctx, enums.PermissionManageInvestment) {
...
func (u *InvestmentUsecase) GetInvestmentHistories(ctx context.Context, dealId, memberId uint64) ([]domain.DealInvestment, float64, error) {
	// Only users with MANAGE_INVESTMENT permission can view investment details
	// if !u.groupAuthUsecase.HasPermission(ctx, enums.PermissionManageInvestment) {
...
func (u *InvestmentUsecase) GetInvestments(ctx context.Context, req *dto.InvestmentDTO) ([]domain.DealInvestment, int64, error) {
...
func (u *InvestmentUsecase) ApproveInvestment(ctx context.Context, id uint64) (uint64, error) {
	...
	investmentID, err := u.investmentRepository.ChangeStatus(ctx, id, enums.TxApprovedStatusApproved)
	if err != nil {
		return 0, err
	}
	...
	go func() {
		...
		defer cancel()
		notification := &_dto.NotificationDTO{
			NotificationType: _enum.NotificationInvestmentApproved,
			TargetID:         investment.DealID,
			AttachData:       []string{fmt.Sprintf("%d", investment.ID)},
			IsMerge:          false,
			OwnerID:          investment.MemberID,
...
func (u *InvestmentUsecase) RejectInvestment(ctx context.Context, id uint64) (uint64, error) {
	...
	investmentID, err := u.investmentRepository.ChangeStatus(ctx, id, enums.TxApprovedStatusRejected)
	if err != nil {
		return 0, err
	}
	...
	go func() {
		...
		notification := &_dto.NotificationDTO{
			NotificationType: _enum.NotificationInvestmentRejected,
			TargetID:         investment.DealID,
			AttachData:       []string{fmt.Sprintf("%d", investment.ID)},
			IsMerge:          false,
			OwnerID:          investment.MemberID,
...
func (u *InvestmentUsecase) canViewInvestment(ctx context.Context, investment *domain.DealInvestment) bool {
...
func (u *InvestmentUsecase) canManageInvestment(ctx context.Context, investment *domain.DealInvestment) bool {
	...
	if investment.Status == enums.TxApprovedStatusApproved {
		return false
	}
