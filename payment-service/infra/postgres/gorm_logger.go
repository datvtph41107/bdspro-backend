...
func (l slogGormLogger) Info(ctx context.Context, msg string, data ...interface{}) {
	...
	logging.WithComponent(ctx, "postgres").Info(fmt.Sprintf(msg, data...))
}
...
func (l slogGormLogger) Warn(ctx context.Context, msg string, data ...interface{}) {
	...
	logging.WithComponent(ctx, "postgres").Warn(fmt.Sprintf(msg, data...))
}
...
func (l slogGormLogger) Error(ctx context.Context, msg string, data ...interface{}) {
	...
	logging.WithComponent(ctx, "postgres").Error(fmt.Sprintf(msg, data...))
}
...
func (l slogGormLogger) Trace(
	...
	case err != nil &&
		...
		sql, rows := fc()
		logging.WithComponent(ctx, "postgres").Error(
	...
	case l.slowThreshold > 0 && elapsed > l.slowThreshold && l.level >= gormlogger.Warn:
		sql, rows := fc()
		logging.WithComponent(ctx, "postgres").Warn(
			"slow database query",
			slog.Duration("duration", elapsed),
	...
	case l.level >= gormlogger.Info:
		sql, rows := fc()
		logging.WithComponent(ctx, "postgres").Info(
			"database query",
			slog.Duration("duration", elapsed),
			slog.Int64("rows", rows),
			slog.String("sql", sql),
